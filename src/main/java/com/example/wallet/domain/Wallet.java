...
public class Wallet {
    ...
    public void addBalance(BigDecimal amount) {
        log.info(">>> [WALLET_ADD] 잔액 가산 시작 - WalletId: {}, Amount: {}", this.walletId, amount);
        ...
        log.info(">>> [WALLET_ADD] 잔액 가산 완료 - New Balance: {}", this.balance);
    }
    ...
    public void deductBalance(BigDecimal amount) {
        log.info(">>> [WALLET_DEDUCT] 잔액 차감 시작 - WalletId: {}, Amount: {}", this.walletId, amount);
        ...
        // [Self-Review] 잔액 부족 검증: 차감액이 현재 잔액보다 큰 경우 예외 처리
        if (this.balance.compareTo(amount) < 0) {
            log.error(">>> [WALLET_DEDUCT] 잔액 부족 실패 - Current: {}, Request: {}", this.balance, amount);
